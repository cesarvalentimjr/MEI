...
class EnhancedBankMatcher:
    ...
    def classify_debit_credit_improved(self, line: str, value_info: Dict) -> Tuple[bool, float]:
        """
        Classificação melhorada de débito/crédito.
        ...
        # 1. Análise de formatação do valor (alta confiança)
        for pattern in self.debit_formatting_patterns:
            if re.search(pattern, value_info['text'], flags=re.IGNORECASE):
                debit_score += 0.4
                confidence_score += 0.3
                break
        
        for pattern in self.credit_formatting_patterns:
            if re.search(pattern, value_info['text'], flags=re.IGNORECASE):
                credit_score += 0.4
                confidence_score += 0.3
                break
        
        # 2. Sinal numérico original (alta confiança)
        if value_info['value'] < 0:
            debit_score += 0.3
            confidence_score += 0.2
        elif value_info['value'] > 0 and ('+' in value_info['text']):
            credit_score += 0.3
            confidence_score += 0.2
        
        # 3. Indicadores textuais fortes (alta confiança)
        for indicator in self.strong_debit_indicators:
            if indicator in line_lower:
                debit_score += 0.35
                confidence_score += 0.25
                break
        
        for indicator in self.strong_credit_indicators:
            if indicator in line_lower:
                credit_score += 0.35
                confidence_score += 0.25
                break
        
        # 4. Indicadores textuais moderados (média confiança)
        for indicator in self.moderate_debit_indicators:
            if indicator in line_lower:
                debit_score += 0.15
                confidence_score += 0.1
                break
        
        for indicator in self.moderate_credit_indicators:
            if indicator in line_lower:
                credit_score += 0.15
                confidence_score += 0.1
                break
        
        # 5. Análise de contexto próximo ao valor
        context_full = context_before + " " + context_after
        
        if any(word in context_full for word in ['saque', 'retirada', 'pagamento', 'tarifa']):
            debit_score += 0.2
            confidence_score += 0.1
        
        if any(word in context_full for word in ['depósito', 'recebimento', 'crédito', 'entrada']):
            credit_score += 0.2
            confidence_score += 0.1
        
        # 6. Padrões específicos do sistema bancário brasileiro
        if re.search(r'pix\s+(enviado|pagto|para)', line_lower):
            debit_score += 0.3
        elif re.search(r'pix\s+(recebido|de)', line_lower):
            credit_score += 0.3
        
        if re.search(r'ted\s+(enviada|para)', line_lower):
            debit_score += 0.3
        elif re.search(r'ted\s+(recebida|de)', line_lower):
            credit_score += 0.3
        
        # 7. Análise de sufixos D/C
        if re.search(r'\bD\b', value_text) or re.search(r'\bD\b', context_after):
            debit_score += 0.4
            confidence_score += 0.3
        elif re.search(r'\bC\b', value_text) or re.search(r'\bC\b', context_after):
            credit_score += 0.4
            confidence_score += 0.3
        
        # Decisão final
        is_debit = debit_score > credit_score
        final_confidence = min(1.0, confidence_score + abs(debit_score - credit_score))
        
        return is_debit, final_confidence
